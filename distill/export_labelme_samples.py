...
def main():
    ...
    ap.add_argument("--verdict", choices=["keep", "delete", "error_keep"], help="可选：仅抽取含该 verdict 标注的图片（不填则任意）")
    ap.add_argument("--only-verdicts", nargs="*", choices=["keep", "delete", "error_keep"], help="只导出这些 verdict 的标注（默认全部）。例：--only-verdicts keep 仅保留 keep 框")
    args = ap.parse_args()
    ...
    for img_id in sel:
        ...
        for a in by_img[img_id]:
            ...
            if args.only_verdicts and v not in args.only_verdicts:
                continue
            by_v[v] += 1
            exported += 1
            x, y, w, h = a["bbox"]
        ...
        json.dump(labelme, open(out_json, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
