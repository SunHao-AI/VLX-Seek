...
多卡说明：
    ...
    模型，按 --queue-batch-size（默认 32）从队列拉取图像，凑满一批处理完 ack 再取
    下一批（queue-batch-size=1 时逐张拉取，负载均衡粒度最细）；进度在进程内跨批次
    累积，每 10 张落盘一次、退出前收尾落盘；主进程统计全部确认后发哨兵，子进程
    优雅退出。各子进程分别写入 <output>.shard<i>.json，全部完成后合并为最终输出。
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "这是消费粒度，与模型推理 batch 无关；落盘频率每 10 张一次、与它解耦："
        "queue-batch-size=1 时逐张拉取、处理完一张立即 ack 再取新图，负载均衡粒度最细；"
...
def _worker_shard(
    ...
    # 从共享队列按 --queue-batch-size 拉取（默认 32）：凑满一批处理完 ack 再取下一批。
